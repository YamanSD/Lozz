...
import { NoDataError, NoDeactivateError, NoDeleteError, NoUpdateError } from "./Errors";
...
export default abstract class BaseController<RawData extends Generic> {
  ...
  /**
   * @returns the firestore document for the ID list of the collection
   */
   ...
   * @protected
   */
  protected async updateServer(data: Generic, id: string) {
    if (!this.canUpdate) {
   ...
   * @protected
   */
  protected async reactivate(id: string) {
    if (!this.canDeactivate) {
      throw new NoDeactivateError();
    }
   ...
   * @protected
   */
  protected async erase(id: string) {
    if (!this.canDelete) {
      throw new NoDeactivateError();
    }
    ...
    await this.updateServer(newData, id);
  }
   ...
   * @protected
   */
  protected async getDocument(id: string): Promise<RawData | undefined> {
    if (this.checkCache(id)) {
   ...
   */
  public async removeId(id: string) {
  ...
  /**
   * Updates the ID list of the collection on server
   */
  public async uploadIds() {
    await this.server.runTransaction(
      async (transaction) => {
        transaction.update(this.idListDocument, {
          data: this.idList
        });
    });
  }
  ...
  /**
   * @returns the ID list of the collection
   */
  public get idList() {
    return this.idListArray;
  }

  /**
   * @returns a list containing all document IDs in a collection
   */
  private async getIdListServer(): Promise<string[]> {
    ...
    return document.data()?.data as string[];
  }
   ...
   * @returns proper initial raw data that can be used for creation
   * @protected
   */
  protected abstract fillDataGaps(data: any): RawData;
