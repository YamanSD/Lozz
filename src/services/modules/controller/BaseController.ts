...
export default class BaseController<RawData extends Generic> {
  ...
  protected removeCache(id: string): void {
    ...
    insert(this.searchEngine, this.getCache(id) as RawData).then(insert_id => {
      if (this.searchEngine !== undefined) {
        remove(this.searchEngine, insert_id).then(() => {
          this.storage.delete(id);
          this.triggerHook();
        });
      }
  ...
  protected async loadSearchData() {
    ...
    for (let id of this.storage.getAllKeys()) {
      ...
      await insert(this.searchEngine, data);
    }
  }
   ...
   * @protected
   */
  protected async updateSearchEngine(data: RawData) {
    if (this.searchEngine === undefined) {
      return;
    }
