...
export default abstract class BaseController<RawData extends Generic> {
  ...
  protected constructor(collection_name: string,
    ...
    this.getIdListServer().then((list) => {
      this.idList = list;
    });
  }
   ...
   */
  public get idListCollectionName() {
   ...
   */
  public get idListDocument() {
    return this.server
      .collection(this.idListCollectionName)
  ...
  public async isIdAvailable(id: string) {
    ...
    return !(id in this.idList) || await this.isErased(id);
  }
   ...
   * @private
   */
  private get idListUpload() {
    return Array.from<string>(this.idList.values());
  }
  ...
  public async uploadIds() {
    await this.server.runTransaction(
      async (transaction) => {
        transaction.set(this.idListDocument, {
          data: this.idListUpload
        });
    });
  }
   ...
   */
  public get idList() {
    return this.idListSet;
  }

  /**
   * @param value new set of IDs
   */
  public set idList(value) {
    this.idListSet = value;
  }

  /**
   * @returns a list containing all document IDs in a collection
   */
  private async getIdListServer(): Promise<Set<string>> {
    const document = await this.idListDocument.get();
